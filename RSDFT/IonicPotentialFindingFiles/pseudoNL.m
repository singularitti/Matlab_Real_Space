...
%
    %n_index = 0;
    n_indexArray=ones(1,indx);
    n_indexArray=cumsum(n_indexArray);
    jArray=1:indx;
%-------------------- Double loop over r and r'
    for i = 1 : indx
      %   index for r
      vnll(n_indexArray,1)=nn(i);
      vnll(n_indexArray,2)=nn(jArray);
      vnll(n_indexArray,3)=total(i,jArray); 
      n_indexArray=n_indexArray+indx;
...
% This adds the size information so that spconvert can be used
    vnll(n_indexArray(1),1)=ndim ;
    vnll(n_indexArray(1),2)=ndim ;
    vnll(n_indexArray(1),3)=0 ;
%%######
